import ResultError, { ResultErrorInput } from "./result-error"

export type ResultValidationError = ResultErrorInput
export type ResultExceptionFactory = (error: ResultValidationError) => unknown

type ResultValidationSource<T> = {
	readonly instance: T
	readonly isFailure: boolean
	readonly errors?: ResultValidationError
}
...
> {
	...
	throwsIfTrue(
		...
		if (this.source.instance === true) this.throwError(error, exceptionFactory)
		return this
	}
	...
	throwsIfFalse(
		...
		if (this.source.instance === false) this.throwError(error, exceptionFactory)
		return this
	}

	throwsIfNull(
		error: ResultValidationError = "RESULT_INSTANCE_NULL",
		exceptionFactory?: ResultExceptionFactory,
	): this {
		if (this.source.instance == null) this.throwError(error, exceptionFactory)
		return this
	}
	...
	throwsIfFailed(error?: ResultValidationError, exceptionFactory?: ResultExceptionFactory): this {
		if (this.source.isFailure) {
			this.throwError(error ?? this.source.errors ?? "RESULT_FAILED", exceptionFactory)
		}
		return this
	}
	...
	private throwError(error: ResultValidationError, exceptionFactory?: ResultExceptionFactory): never {
		if (exceptionFactory) throw exceptionFactory(error)
