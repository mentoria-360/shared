...
export default class Currency extends ValueObject<CurrencyProps, ValueObjectConfig> {
	...
	static create(value: CurrencyInput, config?: ValueObjectConfig): Currency {
		const result = Currency.tryCreate(value, config)
		result.validator.throwsIfFailed()
		return result.instance
	}

	static tryCreate(value: CurrencyInput, config?: ValueObjectConfig): Result<Currency> {
		try {
			...
			if (!/^[A-Z]{3}$/.test(code ?? "")) throw new Error(Currency.INVALID_CURRENCY_CODE)
			if (!symbol) throw new Error(Currency.INVALID_CURRENCY_SYMBOL)
			if (!/^[A-Z]{2}$/.test(countryCode ?? "")) {
				throw new Error(Currency.INVALID_CURRENCY_COUNTRY_CODE)
			}
			if (!name) throw new Error(Currency.INVALID_CURRENCY_NAME)
	...
	private static resolveInput(value: CurrencyInput): CurrencyProps {
		...
		if (!currency) throw new Error(Currency.INVALID_CURRENCY_CODE)
		return currency
	}
}
