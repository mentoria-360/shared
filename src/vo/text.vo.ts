...
export default class Text extends ValueObject<string, TextConfig> {
	...
	protected static readonly DEFAULT_MAX_LENGTH: number = Number.MAX_SAFE_INTEGER

	protected constructor(value: string, config?: TextConfig) {
		super(value, config)
	}

	static create(value: string, config?: TextConfig): Text {
		const result = Text.tryCreate(value, config)
		result.validator.throwsIfFailed()
		return result.instance
	}

	static tryCreate(text: string, config?: TextConfig): Result<Text> {
		try {
