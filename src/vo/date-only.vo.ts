...
export class DateOnly extends ValueObject<string, ValueObjectConfig> {
  ...
  public static tryCreate(
    value: DateOnlyInput | null | undefined,
  ...
  public static tryCreate(
    ...
    try {
      return Result.ok(
        new DateOnly(
          DateOnly.normalize(value as DateOnlyInput),
          resolveVoConfig(metaOrConfig),
        ),
      );
    } catch (error: any) {
  ...
  private static normalize(value: DateOnlyInput): string {
    if (value instanceof Date) {
      if (Number.isNaN(value.getTime())) {
        throw new ValidationError({ code: DateOnly.INVALID_DATE_ONLY });
      }
    ...
    const plainDateMatch = rawValue.match(DateOnly.DATE_REGEX);
    if (plainDateMatch) {
